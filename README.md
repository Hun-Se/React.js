...
- 사용법
  ...
  React에서는 함수를 props로 전달하기 위하여 bind()를 사용한다.
  <a href="https://webisfree.com/2021-04-24/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8-%ED%95%A8%EC%88%98%EC%97%90-bind-%EB%A9%94%EC%86%8C%EB%93%9C-%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0">참고사이트</a>
...
- Thunk : 다른 작업이 완료 될 때 까지 작업을 지연시키는 단순한 함수, 작업 객체를 즉시 반환하지 않는 작업 크리에이터를 작성하기위해 Thunk를 사용한다. 실제 작업 객체를 디스패치 하기전에 다른 코드를 실행할 수 있다.

- next.js
  1. next.js는 CSR(Client Side Rendering)방식인 React 프로젝트를 pre-reloading을 사용하여 미리 데이터가 렌더링된 페이지를 가져올수 있게 하여 SSR(Server Side Rendering) 과 SSG(Static-Site Generate)을 쉽게 구현 할 수 있는 React 프레임워크이다.
  2. next.js는 두번의 렌더링을 통해 문제가 생긴다. 서버에서 가져오는 html을 두번째 렌더링 싸이클에서 가져오기 때문이다.
  3. next.js가 자동적으로 생성하는 사전렌더링은 두번쨰 랜더링 싸이클을 기다리지 않고 첫번째 렌더링 싸이클에서 사전렌더링했던 페이지를 반환한다. 그렇게되면 첫번째 렌더링 싸이클 페이지만 반환하게된다.
  4. 페이지 기반 라우팅 시스템을 지원하기 때문에 폴더 및 route의 생성이 쉽고 편했다.
  5. 또한 [동적폴더명]과 같은 형식으로 dynamic route를 생성 할 수 있어서 직관적이고 편리했다.
  6. CSR에서 구현하기 힘든 SEO(검색최적화)를 쉽게 할 수 있어서 좋았다.
